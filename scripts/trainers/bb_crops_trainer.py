...
def train_model(
    ...
    for e in range(n_epochs):
        ...
        for valid in valid_loader:
            ...
            crops = net.normalize_crops(valid['crops'][:, -1, :])
        ...
        if checkpoints_path:
            current_metric = valid_metrics['fiou_8']/len(valid_loader)
            if  current_metric > best_metric:
                best_metric = current_metric
                torch.save(net.state_dict(), '{}_e{}_fiou_{:.2f}.pt'.format(
                    checkpoints_path, e, best_metric
                ))
